...
推荐优先使用产品里的“系统配置”页面填写 GitHub 和大模型配置。页面会调用后端 `POST /runtime-config`，把配置保存到本地 `data/runtime-config.json`，密钥不会提交到仓库，读取时也只会显示脱敏状态。

`.env` 仍然保留为本地开发和部署兜底方式。本项目的真实密钥不要提交到仓库。需要手动配置时，请在项目根目录创建本地文件：
